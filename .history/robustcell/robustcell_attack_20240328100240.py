...
class graphRobustCell(object):
    ...
    def evaluate(self, surrogate, input_data, modified_adj, labels, idx_train):

        features = input_data
        _,predicted = torch.max(torch.softmax(surrogate.predict(features.cuda(), modified_adj), dim=1), 1)

        results = sklearn.metrics.classification_report(labels[train_obs], predicted.cpu().numpy()[train_obs], return_dict=return_dict)
